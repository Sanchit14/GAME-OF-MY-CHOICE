...
function createDeck() {
  ...
  // create each row
  for (var i = 0; i < rows; i++) {if (window.CP.shouldStopExecution(0)) break;
    var row = {};
    row.cards = [];

    // creat each card in the row
    for (var j = 0; j < cols; j++) {if (window.CP.shouldStopExecution(1)) break;
      var card = {};
      card.isFaceUp = false;
      card.item = key.pop();
      row.cards.push(card);
...
function createRandom() {
  ...
  for (var i = 0; i < matches * 2; i++) {if (window.CP.shouldStopExecution(2)) break;
    pool.push(i); // random numbers
  }

  // generate an array with the random items
  window.CP.exitedLoop(2);for (var n = 0; n < matches; n++) {if (window.CP.shouldStopExecution(3)) break;
